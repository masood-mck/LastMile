...
st.set_page_config(page_title="Xdock Normalized Cost", layout="wide")
...
st.markdown('<div class="app-title">Xdock Cost — Market Comparison</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    view = st.radio(
        "Chart view",
        ["Market comparison", "Over / under payment"],
...
summary["status"] = summary["deviation"].apply(
    lambda d: "Overpaying" if d > 0 else ("Underpaying" if d < 0 else "At baseline")
)

period_label = f"{calendar.month_name[sel_month]} {sel_year}"
STATUS_COLORS = {"Overpaying": MCK_RED, "Underpaying": MCK_GREEN, "At baseline": MCK_GRAY}
...
_kpis = [
    ...
    ("Overpaying", str(int((summary["status"] == "Overpaying").sum())), "markets"),
    ("Underpaying", str(int((summary["status"] == "Underpaying").sum())), "markets"),
    ("Worst overpay", f"+{_worst['deviation_pct']:.0f}%", _worst["MARKET"]),
]
_cards = "".join(
...
if view == "Market comparison":
    st.subheader("Cost per stop vs. normalized vs. cleansheet — by xdock")
    ...
    st.subheader("Over / under payment — vs. each Fill DC's own baseline")
    st.caption("Distance from each DC's own baseline. **Red = overpaying**, **green = underpaying**.")
    ...
    fig2 = px.bar(
        ...
        labels={"deviation_pct": "% difference from DC baseline", "status": ""},
    )
    fig2.update_traces(
        textposition="outside",
        cliponaxis=False,
        hovertemplate=(
            "<b>%{customdata[0]}</b> (DC %{customdata[1]})<br>%{x:+.1f}% vs DC base<br>"
...
with st.expander("Detail table & download"):
    ...
    st.download_button(
        ...
        file_name=f"xdock_cost_{sel_year}_{sel_month:02d}.csv",
        mime="text/csv",
    )
