...
if market_options:
    ...
    if not row_df.empty:
        ...
        cols[4].metric("Records With CPS", f"{int(row.get('Records With CPS', 0)):,}", row.get("Confidence", ""))
...
tab0, tab00, tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs(
    [
        "Method Flow",
        "Methodology",
        "Actual vs expected",
...
A Quantile Gradient Boosting model estimates:

- P10 Expected CPS
- P50 Expected CPS
- P90 Expected CPS

P50 is treated as the primary expected cost.

P10 and P90 define the expected operating range for comparable markets.
...
- Bucket size is based on Records With CPS, so larger boxes represent more operational evidence.
            ...
            st.dataframe(
                ...
                hide_index=True,
                column_config={
                    "Records_With_CPS": st.column_config.NumberColumn(format="%,d"),
                    "Median_Model_Gap": st.column_config.NumberColumn(format="%.1f%%"),
                    "Median_Normalized_Gap": st.column_config.NumberColumn(format="%.1f%%"),
                    "Median_Cleansheet_Gap": st.column_config.NumberColumn(format="%.1f%%"),
                    "Median_Three_Signal_Score": st.column_config.NumberColumn(format="%.3f"),
                },
            )
